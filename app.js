...
async function loadAll(){
  ...
  if(!items){ items = MENU_FALLBACK.items || []; }
  if(!conf){ conf = CONFIG_FALLBACK; }
  state.items = items; state.conf = conf;
...
if(el.dipMinus){ if(el.dipMinus){ el.dipMinus.onclick=()=>{ if(state.select.dipQty>0){ state.select.dipQty--; } } el.dipQty.textContent=state.select.dipQty; } };
if(el.dipPlus){ if(el.dipPlus){ el.dipPlus.onclick=()=>{ state.select.dipQty++; } } el.dipQty.textContent=state.select.dipQty; };
