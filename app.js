...
function dedupeSeparators(){
  ...
  seps.forEach(node => {
    if (prev && prev.nextElementSibling === node && prev.classList.contains('section-sep') && node.classList.contains('section-sep')) {
      ...
      else if (prev && prev.classList.contains('auto')) prev.remove();
    } else {
      prev = node;
    }
  });
...
function updateModeUI(){
  if(state.mode==='delivery'){
    ...
    el.deliveryMode.textContent = 'Режим: Самовывоз — ' + (state.conf.pickup && state.conf.pickup.address ? state.conf.pickup.address : '');
...
function updateGeoUI(){
  ...
  if(g.status==='inside'){
    ...
    const pickupAddr = state.conf.pickup && state.conf.pickup.address ? state.conf.pickup.address : '';
